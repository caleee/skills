...
> 本 skill 仅给文字清单，不带独立模板文件；照此 5 段 + 验证段写即可。

1. **Context** — 图纸/现状/分支（实测数据、用户诉求、目标一句话）
2. **模块归类表** — 图规范/现状差距/关联文件（每行一模块，含份数/薄/核心问题/目标）
3. **大计划** — 目标/顺序/依赖/数据约定/约束（阶段表 + Gate）
4. **各模块小计划** — 改动点 + 验收（每模块：文件|现状|动作|手法|目标行|验收）
5. **进度表** — 每行一模块：计划|状态|产出；状态用五态机，完成后补“☑ 已完成 (YYYY-MM-DD)”
6. **执行记录** — 追加式：日期|动作|备注
7. **验证** — 每 plan 末尾：阶段门禁 + 终验命令（`ls`/`build`/`git diff` 等）
